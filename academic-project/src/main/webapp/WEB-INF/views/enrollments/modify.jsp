		...
		<jsp:include page="../include/header.jsp" />
			<div class="row">
				<ol class="breadcrumb">
					...
					<li class="active">수강신청수정</li>
				</ol>
			</div>
			<form id="form-modify-enroll" action="/enrollments/modify" role="form" method="post">
				...
				<div class="form-group">
					...
					<select id="select-enrolllcode" name="lcode" class="form-control">
						<c:forEach var="cvo" items="${clist}">
							<option value="${cvo.lcode}" <c:out value="${cvo.lcode eq evo.lcode?'selected':''}"/>>${cvo.lcode}: ${cvo.lname}</option>
						</c:forEach>
					</select>
				</div>
				<div class="form-group">
					...
					<select id="select-enrollscode" name="scode" class="form-control">
						<c:forEach var="svo" items="${slist}">
							<option value="${svo.scode}" <c:out value="${svo.scode eq evo.scode?'selected':''}"/>>${svo.scode}: ${svo.sname}</option>
						</c:forEach>
					</select>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<input id="input-grade" type="text" name="grade" class="form-control" value="${evo.grade}">
				</div>
...
<script>
	$(document).ready(function() {
		var frm = document.getElementById('form-modify-enroll');
		...
		frm['enroll_yy'].value = <%= yy%>;
		frm['enroll_mm'].value = <%= mm%>;
		setDay();
		frm['enroll_dd'].value = <%= dd%>;
	});
